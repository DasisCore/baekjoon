...
before = input()
before = list(map(str, before)) #입력된 문자열을 리스트화

after = []
for i in before:
    after.append(alpha.index(i)+1) #인덱스를 이용해 문자열 리스트를 해당하는 숫자로 변환

answer = 0
for i in range(len_str):
    answer += after[i]*(r**i) #해쉬함수 식
answer = answer % m #mod는 나머지 연산을 의미함!
print(answer)
