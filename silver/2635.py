...
# 5. 첫 번째 수로 100이 주어질 때, 두 번째 수로 60을 선택하여 위의 규칙으로 수들을 만들면 7개의 수들 100, 60, 40, 20, 20 , 0, 20이 만들어진다. 그리고 두 번째 수로 62를 선택하여 위의 규칙으로 수들을 만들면 8개의 수들 100, 62, 38, 24, 14, 10, 4, 6이 만들어진다. 위의 예에서 알 수 있듯이, 첫 번째 수가 같더라도 두 번째 수에 따라서 만들어지는 수들의 개수가 다를 수 있다.
...
li = [n]
dic = {}
# n//2 이하는 넣어도 3개 이상 못가므로
for i in range(n//2, n+1): 
    check = list(li) # 할당이 안되도록 새로운 리스트로 생성
    check.append(i) # n//2부터 n까지 전부 확인
    while check[-1] >= 0:
        a = check[-2] - check[-1]
        check.append(a)
    dic[len(check[:-1])] = check[:-1] # -가 나온 부분 제거

print(max(dic)) # 출력 양식에 맞추어 출력
print(*dic[max(dic)]) 