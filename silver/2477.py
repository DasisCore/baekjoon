...
# 참외밭의 규칙성
# length[0] > length[2] and length[1] > length[5] 인 구간을 찾아
# 출발점을 계속 돌린다.
while True:
    if length[0] > length[2] and length[1] > length[5]:
        break
    else:
        a = length[0]
        length.remove(a)
...
# 참외밭을 두 분할로 나누어 구한다.
print(((length[5] * length[0]) + (length[2] * length[3])) * n)
...
#         small.append(num)
#
# a = length[go.index(big[0])] #가장 큰 가로
# b = length[go.index(big[1])] #가장 큰 세로 // 1234의 순서이므로
# if length[go.index(small[0])] <= a // 2: #가로
#     c = length[go.index(small[0])]
# else:
#     c = a -  length[go.index(small[0])]
# if length[go.index(small[1])] <= b // 2: #세로
#     d = length[go.index(small[1])]
# else:
#     d = b - length[go.index(small[1])]
#
# print(((a * b) - (c * d)) * n)
