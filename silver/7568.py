...
li = []
for i in range(num): # 입력값을 2차원 리스트로 형성
    a = list(map(int, input().split(" ")))
    li.append(a) 

answer = [] #등수를 넣을 리스트 
for i in range(num): #1번 선수부터 시작
    cnt = 1
    for j in range(num): #1번 선수부터 자기 자신을 포함해, 자신보다 작은 사람 찾기
        if li[i][0] < li[j][0] and li[i][1] < li[j][1]: #몸무게, 키 비교
            cnt += 1
        plus = cnt       
    answer.append(plus) 

print(*answer) #[]를 없앤후 출력
