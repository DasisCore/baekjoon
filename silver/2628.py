...
p_w = [] #자른 후의 종이 크기가 담길 리스트
p_h = []
# 0과 h, w 사이에 자를 크기가 존재하므로 이를 이용하여 자른다.
for i in range(len(liw)-1):
    p_w.append(liw[i+1] - liw[i])

for i in range(len(lih)-1):
    p_h.append(lih[i+1] - lih[i])

print(max(p_w) * max(p_h))
